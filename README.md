...
Quick tips:
- First connection without a fingerprint will still work with a self-signed cert, but the client logs a warning and prints the certificate fingerprint. Copy it and reconnect with `--cert-fingerprint` for security.
- Certificates generated by the listener include SANs for `localhost`, `127.0.0.1`, and `::1` for local testing.
...
- TLS: Enforces TLS 1.3 for strong, modern cryptography.
...
Examples:
- First connect (self-signed, no fingerprint yet):
  ```bash
  ./gotsr 127.0.0.1:8443 1
  # ⚠️ Logs: Self-signed certificate detected…
  # ⚠️ Shows: Certificate fingerprint: <sha256>
  ```
- Secure reconnect with pinning:
  ```bash
  ./gotsr --cert-fingerprint <sha256> 127.0.0.1:8443 1
  ```

## Notes
- Protocol: TLS 1.3 over TCP only (no HTTP).
- Listener uses an ephemeral self-signed cert by default and includes SANs for `localhost`, `127.0.0.1`, and `::1`.
- Shared secret authentication uses cryptographically secure random 32-byte (hex-encoded) secrets and timing-safe comparisons on the server.
- Certificate fingerprints are SHA256 of the server certificate (DER encoded).
